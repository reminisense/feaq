...
(function ()
{
    ...
    app.controller('processqueueController', function ($scope, $http)
    {
        ...
        var day = current_date.getDate() < 10 ? '0' + current_date.getDate()
          : current_date.getDate();
        var month = (current_date.getMonth() + 1) < 10 ? '0'
...
//            });
            getResponseResetValues(url, null, null, function ()
            {
                ...
                $scope.isCalling = ($scope.date != $scope.today
                && ($scope.timebound_numbers.length
                + $scope.uncalled_numbers.length == 0));
            });
        };
        ...
        $scope.checkIn = function (transaction_number)
        {
            ...
            if (uncalled_number != null && !uncalled_number.checked_in) {
                if (confirm('Are you sure you want to check in this number?')) {
                    transaction_number = transaction_number != undefined
                      ? transaction_number : angular.element(
                        document.querySelector('#selected-tnumber')).val();
                    getResponseResetValues(
                      '/processqueue/checkin-transaction/' + transaction_number,
                      function ()
                      {
                          $scope.issue_call_number = null;
...
//                          $scope.updateBroadcast();
        ...
        getUncalledNumber = function (transaction_number)
        {
            for (index in $scope.uncalled_numbers) {
                if ($scope.uncalled_numbers[index].transaction_number
                  == transaction_number) {
        ...
        $scope.callNumber = function (transaction_number, callback)
        {
            ...
            transaction_number = transaction_number != undefined
              ? transaction_number : angular.element(
                document.querySelector('#selected-tnumber')).val();
            getResponseResetValues(
              pq.urls.process_queue.call_number_url + transaction_number + '/'
              + pq.ids.terminal_id, function ()
              {
        ...
        $scope.serveNumber = function (transaction_number, callback)
        {
            ...
            if ($scope.called_numbers[i].verified_email != undefined
              && $scope.called_numbers[i].verified_email) {
                if ($scope.called_numbers_rating[i] == undefined) {
                    var rating = 3;
                } else {
                    var rating = $scope.called_numbers_rating[i];
                }
                $http.get('/rating/userratings/' + rating + '/'
                  + $scope.called_numbers[i].email + '/' + $scope.terminal_id
                  + '/' + 2 + '/' + transaction_number);
            }

            $scope.isProcessing = true;
            getResponseResetValues(
              pq.urls.process_queue.serve_number_url + transaction_number,
              function ()
              {
...
//                  $scope.updateBroadcast();
        ...
        $scope.dropNumber = function (transaction_number)
        {
            ...
            $scope.isProcessing = true;
            getResponseResetValues(
              pq.urls.process_queue.drop_number_url + transaction_number,
              function ()
              {
                  pq.jquery_functions.remove_from_called(transaction_number);
                  $scope.updateBroadcast();
        ...
        $scope.stopProcessQueue = function ()
        {
            ...
            $scope.isCalling = true;
            $scope.progress_max = $scope.progress_max > 0 ? $scope.progress_max
              : $scope.called_numbers.length;
            ...
            $http.post('/processqueue/stop-queue', {ids: JSON.stringify(ids)})
              .success(function ()
              {
        ...
        {
            $http.post(
              pq.urls.issue_numbers.issue_specific_url + pq.ids.service_id + '/'
              + pq.ids.terminal_id, {priority_number: priority_number})
              .success(function (response)
              {
                  if (response.error) {
        ...
        $scope.issueOrCall = function ()
        {
            ...
            if ($scope.timebound_numbers.length == 0
              && $scope.uncalled_numbers.length == 0) {
                if (!checkTextfieldErrors($scope.issue_call_number)
                  && $scope.issue_call_number !== undefined) {
        ...
        $scope.moveToday = function (transaction_number)
        {
            if ($scope.timebound_numbers.length > 0
              || $scope.uncalled_numbers.length > 0) {
                var transaction_number = transaction_number != undefined
                  ? transaction_number : angular.element(
                    document.querySelector('#selected-tnumber')).val();
                var data = {
                ...
                for (i = 0; i < $scope.timebound_numbers.length; i++) {
                    if ($scope.timebound_numbers[i].transaction_number
                      == transaction_number) {
                ...
                for (i = 0; i < $scope.uncalled_numbers.length; i++) {
                    if ($scope.uncalled_numbers[i].transaction_number
                      == transaction_number) {
        ...
        {
            getResponseResetValues(
              '/processqueue/update-broadcast/' + pq.ids.business_id,
              function ()
              {
                  $scope.sendWebsocket();
              });
        ...
        checkTextfieldErrors = function (priority_number)
        {
            var issueController = angular.element(
              document.querySelector('#moreq')).scope();
            issueController.priority_number = $scope.next_number;
            return issueController.checkIssueSpecificErrors(priority_number,
              $scope.number_limit, false);
        };

        //non scope functions
        getResponseResetValues = function (
          url,
          successFunc,
          errorFunc,
          finallyFunc
        ...
        {
            next_number = angular.element(
              document.querySelector('#selected-tnumber')).val();
            is_uncalled = pq.jquery_functions.find_in_numbers_array(next_number,
              $scope.uncalled_numbers);
            is_timebound = pq.jquery_functions.find_in_numbers_array(
              next_number, $scope.timebound_numbers);
            ...
            if ($scope.timebound_numbers.length == 0
              && $scope.uncalled_numbers.length == 0) {
                ...
                if (next_number == 0) {
                    ...
                    if (is_uncalled.length == 0 && is_timebound.length == 0) {
                        pq.jquery_functions.remove_and_update_dropdown(
                          next_number);
                    } else {
                        if ($scope.timebound_numbers.length > 0
                          && is_timebound.length == 0) {
                            pq.jquery_functions.select_number(
                              $scope.timebound_numbers[0].transaction_number,
                              $scope.timebound_numbers[0].priority_number,
                              $scope.timebound_numbers[0].name,
        ...
        getIndex = function (transaction_number)
        {
            for (var i = 0; i < $scope.called_numbers.length; i++) {
                if ($scope.called_numbers[i].transaction_number
                  == transaction_number) {
                    return i;
                    break;
                }
        ...
        $scope.getAllowedBusinesses = function ()
        {
            ...
            $http.get('/business/allowed-businesses/' + pq.ids.business_id)
              .success(function (response)
              {
                  if (response.allowed_businesses
                    && response.allowed_businesses.length != 0) {
                      $scope.allowed_businesses = response.allowed_businesses;
                      for (var index in $scope.allowed_businesses) {
                          if ($scope.allowed_businesses[index].service_id
                            != pq.ids.service_id) {
                              $('#allowed-businesses')
                                .append('<option value="'
        ...
        $scope.issueToOther = function (service_id, transaction_number)
        {
            ...
            $scope.serveNumber(transaction_number, function ()
            {
                $http.post('/issuenumber/issue-other', data)
                  .success(function (response)
                  {
                      ...
                      $('#forward-success')
                        .html(
                          '<p class="forward-num">Forward successful. The priority number given is </p><h2>'
...
//                      }));
        ...
        {
            var new_number = $scope.number_prefix + newValue
              + $scope.number_suffix;
            for (index in $scope.unprocessed_numbers) {
                if (new_number
                  == $scope.unprocessed_numbers[index].priority_number) {
                    $scope.issue_call_number = oldValue;
                    $scope.issue_call_error = 'Number ' + new_number
                      + ' is still active. ';
                    setTimeout(function ()
                    {
                        $scope.issue_call_error = '';
            ...
            if ($scope.number_limit != null && (newValue
              > $scope.number_limit)) {
                $scope.issue_call_error
                  = 'Priority number is greater than the limit. ';
                setTimeout(function ()
                {
                    $scope.issue_call_error = '';
                }, 3000);
