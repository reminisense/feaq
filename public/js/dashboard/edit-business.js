    ...
    app.controller('editBusinessController', function ($scope, $http, $filter)
    {
        ...
        $scope.getServiceQueueSettings = function (service_id, service_name)
        {
            $http.get('/queuesettings/allvalues/' + service_id).success(function (response)
            {
                if (response.success == 1) {
                    ...
                    // pacing records
                    $scope.service_settings.pacingRecords = response.pacingRecords;
                    $scope.service_settings.pacingStatus = response.pacingStatus;
        ...
        setServiceBoxes = function ()
        {
            $http.get('/broadcast/service-boxes').success(function (response)
            {
                ...
                $scope.service_boxes.box4_service = response.box4;
                $scope.service_boxes.box5_service = response.box5;
                $scope.service_boxes.box6_service = response.box6;
                $scope.service_boxes.box7_service = response.box7;
                $scope.service_boxes.box8_service = response.box8;
                $scope.service_boxes.box9_service = response.box9;
                $scope.service_boxes.box10_service = response.box10;
                $scope.service_boxes.box11_service = response.box11;
                $scope.service_boxes.box12_service = response.box12;
                $scope.service_boxes.box13_service = response.box13;
        ...
        $scope.saveBusinessDetails = function (business)
        {
            ...
            if ($scope.sms_blank_ahead == 1 && $scope.input_sms_field == "") {
                ...
                if (($scope.input_sms_field % 1) != 0) {
                    ...
                    if ($scope.input_sms_field <= 0 && $scope.input_sms_field != "") {
        ...
        $scope.saveBroadcastSettings = function (business_id)
        {
            ...
            $http.post('/broadcast/save-settings', {
                ...
                box1_service: $scope.service_boxes.box1_service,
                box2_service: $scope.service_boxes.box2_service,
                box3_service: $scope.service_boxes.box3_service,
                box4_service: $scope.service_boxes.box4_service,
                box5_service: $scope.service_boxes.box5_service,
                box6_service: $scope.service_boxes.box6_service,
                box7_service: $scope.service_boxes.box7_service,
                box8_service: $scope.service_boxes.box8_service,
                box9_service: $scope.service_boxes.box9_service,
                box10_service: $scope.service_boxes.box10_service,
                box11_service: $scope.service_boxes.box11_service,
                box12_service: $scope.service_boxes.box12_service,
                box13_service: $scope.service_boxes.box13_service,
                    ...
                    (Math.random() * 10000) + 1)).success(function (response)
                {
                    ...
                    { //user click on remove text
                        ...
                        if (qx < 1) {
                            ...
                            e.preventDefault();
                            $('.q-nums-wrap .qbox:last-child').remove();
                        }
                    });
        ...
        $scope.setRemainingCharacter = (function ()
        {
            ...
            if ($('#lbl-ticker2').css('visibility') == 'hidden') {
                $('#lbl-ticker2').css('visibility', 'visible');
            }
            if ($('#lbl-ticker3').css('visibility') == 'hidden') {
                $('#lbl-ticker3').css('visibility', 'visible');
            }
            if ($('#lbl-ticker4').css('visibility') == 'hidden') {
                $('#lbl-ticker4').css('visibility', 'visible');
            }
            if ($('#lbl-ticker5').css('visibility') == 'hidden') {
                $('#lbl-ticker5').css('visibility', 'visible');
            }
