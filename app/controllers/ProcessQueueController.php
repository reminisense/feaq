...
class ProcessQueueController extends BaseController
{
    ...
    public function getTerminal($terminal_id)
    {
        ...
        return $view->with('body', 'processq')
          ...
          ->with('group_name', Grouping::getGroupName(Service::getGroupIdByService($service_id)))
    ...
    public function getCallnumber($transaction_number, $terminal_id)
    {
        try {
            ...
            if ($terminal_transaction->time_called != 0) {
                return json_encode([
                  'error' => 'Number ' . $priority_queue->priority_number
                    . ' has already been called. Please call another number.'
                ]);
            } else {
    ...
    public function getNextNumber($service_id)
    {
        ...
        return json_encode([
          'next_number' => $all_numbers->number_prefix . $all_numbers->next_number . $all_numbers->number_suffix
        ]);
    }
    ...
    public function getCheckinTransaction($transaction_number)
    {
        if (TerminalTransaction::where('transaction_number', '=', $transaction_number)->exists()) {
            ...
            TerminalTransaction::where('transaction_number', '=', $transaction_number)
              ->update(['time_checked_in' => $time_checked_in]);
            return json_encode([
              'success'            => 1,
    ...
    public function getGroupKiosk($group_id){
        $business_id = Grouping::getBusinessId($group_id);
