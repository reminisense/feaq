...
class ProcessQueue extends Eloquent
{
    ...
    public static function issueNumber(
      ...
      $confirmation_code = null
        ...
        $priority_number = QueueSettings::numberPrefix($service_id) . $priority_number
          . QueueSettings::numberSuffix($service_id);

        $user_id = $user_id == null ? Helper::userId() : $user_id;
        //$user_id = $queue_platform != 'web'? $user_id : 0;

        $track_id = PriorityNumber::createPriorityNumber($service_id, $number_start, $number_limit, $last_number_given,
          $current_number, $date);
        $confirmation_code = $confirmation_code == null ? strtoupper(substr(md5($track_id), 0, 4)) : $confirmation_code;
        $transaction_number = PriorityQueue::createPriorityQueue($track_id, $priority_number, $confirmation_code,
          $user_id, $queue_platform);
        TerminalTransaction::createTerminalTransaction($transaction_number, $time_queued, $terminal_id);
        Analytics::insertAnalyticsQueueNumberIssued($transaction_number, $service_id, $date, $time_queued, $terminal_id,
          $queue_platform); //insert to queue_analytics
        $number = array(
    ...
    public static function issueMultiple(
      $service_id,
      $first_number,
      $range,
      $date = null,
      ...
      $user_id = null
        ...
        for ($i = 1; $i <= $range; $i++) {
            $track_id = PriorityNumber::createPriorityNumber($service_id, $number_start, $number_limit,
              $last_number_given, $current_number, $date);
    ...
    public static function callTransactionNumber($transaction_number, $user_id, $terminal_id)
    {
        if (is_numeric($terminal_id)) {
            ...
            $time_called = time();
            $login_id = TerminalManager::hookedTerminal($terminal_id)
              ? TerminalManager::getLatestLoginIdOfTerminal($terminal_id) : 0;
            TerminalTransaction::updateTransactionTimeCalled($transaction_number, $login_id, $time_called,
              $terminal_id);
            Analytics::insertAnalyticsQueueNumberCalled($transaction_number, $pn->service_id, $pn->date, $time_called,
              $terminal_id, $pq->queue_platform); //insert to queue_analytics
            self::saveNumbersToJSON($pn->service_id, $terminal_id, $pq->priority_number);
            Notifier::sendNumberCalledNotification($transaction_number,
              $terminal_id); //notifies users that his/her number is called
            Notifier::sendPushNotification($transaction_number, $terminal_name, $service_name,
              'call'); // PAG send push notifications
            return json_encode([
              'success' => 1,
              /*'numbers' => ProcessQueue::allNumbers(Terminal::serviceId($terminal_id))*/
            ]); //ARA removed all numbers to prevent redundant database query
        } else {
    ...
    public static function processNumber($transaction_number, $process, $terminal_id = null, $user_id = null)
    {
        ...
        if ($transaction->time_removed == 0 && $transaction->time_completed == 0) {
            ...
            if ($process == 'serve') {
                ...
                Analytics::insertAnalyticsQueueNumberServed($transaction_number, $priority_number->service_id,
                  $priority_number->date, $time, $terminal_id,
                  $priority_queue->queue_platform); //insert to queue_analytics
                ...
                if ($process == 'remove') {
                    ...
                    Analytics::insertAnalyticsQueueNumberRemoved($transaction_number, $priority_number->service_id,
                      $priority_number->date, $time, $terminal_id,
                      $priority_queue->queue_platform); //insert to queue_analytics
                    Notifier::sendPushNotification($transaction_number, "", "",
                      $process); // PAG send push notifications
                }
            }
        } else {
            return json_encode(array(
              'error' => 'Number ' . $pnumber
                . ' has already been processed. If the number still exists, please reload the page.'
            ));
        }
    ...
    public static function segregatedNumbers($numbers, $service_id, $terminal_id)
    {
        ...
        foreach ($numbers as $number) {
            ...
            foreach ($records as $record) {
                ...
                foreach ($form_fields as $count => $field) {
                    ...
                    if ($field['field_data']['label'] != $label[$count]) {
                        $content->form_data->{$field['field_data']['label']} = empty($arr) ? "N/A"
                          : $content->form_data->{$label[$count]};
                        unset($content->form_data->{$label[$count]});
                    } else {
                        $content->form_data->{$label[$count]} = empty($arr) ? "N/A"
                          : $content->form_data->{$label[$count]};
                    }
                }
            ...
            if (!$called && !$removed && $timebound) {
                ...
                if (!$called && !$removed && $terminal_specific_calling
                  && ($number->terminal_id == $terminal_id
                    || $number->terminal_id == 0)
                ) {
                    ...
                    if (!$called && !$removed && (!$terminal_specific_calling || $terminal_id == null)) {
                        if ($number->queue_platform == 'remote' && $checked_in == false && $checkin_status == false) {
                            $unchecked_numbers[] = array(
                              'transaction_number' => $number->transaction_number,
                              'queue_platform'     => $number->queue_platform,
        ...
        $priority_numbers->number_limit = QueueSettings::numberLimit($service_id);
        $priority_numbers->next_number = ProcessQueue::nextNumber($priority_numbers->last_number_given,
          $priority_numbers->number_start, $priority_numbers->number_limit, $priority_numbers->number_prefix,
          $priority_numbers->number_suffix);
        $priority_numbers->current_number = $called_numbers ? $called_numbers[key($called_numbers)]['priority_number']
          : 0;
        ...
        $after_next = ProcessQueue::nextNumber($priority_numbers->number_prefix . $priority_numbers->next_number
          . $priority_numbers->number_suffix, $priority_numbers->number_start, $priority_numbers->number_limit,
          $priority_numbers->number_prefix, $priority_numbers->number_suffix);
        while (ProcessQueue::queueNumberActive($service_id, $priority_numbers->next_number, $after_next)) {
            ...
            if ($after_next == $priority_numbers->number_start) {
                break;
            }
            $after_next = ProcessQueue::nextNumber($priority_numbers->number_prefix . $after_next
              . $priority_numbers->number_suffix, $priority_numbers->number_start, $priority_numbers->number_limit,
              $priority_numbers->number_prefix, $priority_numbers->number_suffix);
        }

        $priority_numbers->unprocessed_numbers = array_merge($priority_numbers->uncalled_numbers,
          $priority_numbers->called_numbers);
        usort($priority_numbers->unprocessed_numbers, array(
          'ProcessQueue',
          'sortUnprocessedNumbers'
        ));
        $priority_numbers->unprocessed_numbers = array_merge($priority_numbers->timebound_numbers,
          $priority_numbers->unprocessed_numbers);
        ...
        if ($prefix != '' || $suffix != '') {
            ...
            if ($suffix != '') {
                ...
                if ($suffix_position >= 1) {
                    $last_number_given = substr($last_number_given, 0, (0 - strlen($suffix)));
                } elseif (is_numeric($last_number_given)) {
                    $last_number_given = $number_limit;
                }
            }
    ...
    public static function getServiceProperties($service_id, $date = null)
    {
        ...
        $properties->next_number = ProcessQueue::nextNumber($properties->last_number_given, $properties->number_start,
          $properties->number_limit, $properties->number_prefix, $properties->number_suffix);
    ...
    public static function updateBusinessBroadcast($business_id)
    {
        ...
        $services = Service::where('branch_id', '=', $first_branch->branch_id)
          ->get();
        $all_numbers = null;
        $all_service_numbers = [];
        foreach ($services as $service) {
            ...
            $all_service_numbers[$service->service_id]
              = clone $service_numbers; //because php passes objects by reference
            if ($all_numbers) {
                $all_numbers->called_numbers = array_merge($all_numbers->called_numbers,
                  $service_numbers->called_numbers);
                $all_numbers->uncalled_numbers = array_merge($all_numbers->uncalled_numbers,
                  $service_numbers->uncalled_numbers);
                $all_numbers->processed_numbers = array_merge($all_numbers->processed_numbers,
                  $service_numbers->processed_numbers);
                $all_numbers->timebound_numbers = array_merge($all_numbers->timebound_numbers,
                  $service_numbers->timebound_numbers);
                usort($all_numbers->called_numbers, array(
                  'ProcessQueue',
                  'sortCalledNumbers'
                ));
            } else {
...
//    ProcessQueue::saveTerminalNumbersToJSON($business_id, $all_numbers);
        ...
        if ($all_numbers) {
            ...
            for ($counter = 1; $box_count <= $max_count; $counter++) {
                if ($counter <= count($numbers)) {
                    ...
                    if (!in_array($numbers[$index]['transaction_number'], $existing)
                      || $number == ''
                    ) { //check if same number already exists
                        $existing[] = $numbers[$index]['transaction_number'];
                        $box = 'box' . $box_count;
                        $boxes->$box->number = $number;
                        $boxes->$box->service = isset($numbers[$index]['service_name'])
                          ? $numbers[$index]['service_name'] : ''; //ARA Added service name for multiple services
                        $boxes->$box->terminal = isset($numbers[$index]['terminal_name'])
                          ? $numbers[$index]['terminal_name'] : '';
                        $boxes->$box->rank = isset($numbers[$index]['box_rank']) ? $numbers[$index]['box_rank']
                          : ''; // Added by PAG
                        $boxes->$box->color = isset($numbers[$index]['color']) ? $numbers[$index]['color']
                          : ''; // Added by PAG
                        $boxes->$box->user = (isset($boxes->show_names) && $boxes->show_names)
                        && isset($numbers[$index]['name']) ? $numbers[$index]['name'] : ''; //Added by ARA
                        $box_count++;
                    }
                } else {
                    $box = 'box' . $box_count;
    ...
    public static function saveServiceNumbersToJSON($business_id, $all_service_numbers)
    {
        ...
        if ($all_service_numbers) {
            ...
            foreach ($all_service_numbers as $service_id => $service_numbers) {
                ...
                $boxes->services->$service_id->get_num = $service_numbers->number_prefix . $service_numbers->next_number
                  . $service_numbers->number_suffix;
                ...
                for ($counter = 1; $counter <= $check_in_display; $counter++) {
                    ...
                    $uncalled_number = isset($service_numbers->uncalled_numbers[$index])
                      ? $service_numbers->uncalled_numbers[$index] : null;
                    $boxes->services->$service_id->queue_now->$queue_now_box = new stdClass();
                    $boxes->services->$service_id->queue_now->$queue_now_box->number = $uncalled_number
                      ? $uncalled_number['priority_number'] : '';
                    $boxes->services->$service_id->queue_now->$queue_now_box->on_standby = $uncalled_number
                      ? $uncalled_number['checked_in'] : '';
                    $boxes->services->$service_id->queue_now->$queue_now_box->rank = $uncalled_number
                      ? $uncalled_number['service_id'] : '';
                    $boxes->services->$service_id->queue_now->$queue_now_box->service = $uncalled_number
                      ? $uncalled_number['service_name'] : '';
                }
                    ...
                    if ($counter <= count($numbers)) {
                        ...
                        if (!in_array($numbers[$index]['transaction_number'], $existing)
                          || $number == ''
                            ...
                            $boxes->services->$service_id->$box->service = isset($numbers[$index]['service_name'])
                              ? $numbers[$index]['service_name'] : ''; //ARA Added service name for multiple services
                            $boxes->services->$service_id->$box->terminal = isset($numbers[$index]['terminal_name'])
                              ? $numbers[$index]['terminal_name'] : '';
                            $boxes->services->$service_id->$box->rank = isset($numbers[$index]['box_rank'])
                              ? $numbers[$index]['box_rank'] : ''; // Added by PAG
                            $boxes->services->$service_id->$box->color = isset($numbers[$index]['color'])
                              ? $numbers[$index]['color'] : ''; // Added by PAG
                            $boxes->services->$service_id->$box->user = (isset($boxes->show_names)
                              && $boxes->show_names)
                            && isset($numbers[$index]['name']) ? $numbers[$index]['name'] : ''; //Added by ARA
                            $box_count++;
                        }
                        ...
                        $boxes->services->$service_id->$box->service
                          = ''; //ARA Added service name for multiple services
    ...
    public static function saveTerminalNumbersToJSON($business_id, $all_numbers)
    {
        if ($all_numbers) {
            ...
            foreach ($all_terminals as $terminal) {
                ...
                $boxes->terminals->$terminal['terminal_id']->queue_now
                  = $boxes->services->$terminal['service_id']->queue_now;
            }
            ...
            foreach ($numbers as $index => $number) {
                if ($boxes->terminals->$number['terminal_id']->box_count <= $max_count) {
                    ...
                    if (!isset($boxes->terminals->$number['terminal_id']->$box)) {
                        $boxes->terminals->$number['terminal_id']->$box = new stdClass();
                    }
                    $boxes->terminals->$number['terminal_id']->$box->number = $numbers[$index]['priority_number'];
                    $boxes->terminals->$number['terminal_id']->$box->service = isset($numbers[$index]['service_name'])
                      ? $numbers[$index]['service_name'] : ''; //ARA Added service name for multiple services
                    $boxes->terminals->$number['terminal_id']->$box->terminal = isset($numbers[$index]['terminal_name'])
                      ? $numbers[$index]['terminal_name'] : '';
                    $boxes->terminals->$number['terminal_id']->$box->rank = isset($numbers[$index]['box_rank'])
                      ? $numbers[$index]['box_rank'] : ''; // Added by PAG
                    $boxes->terminals->$number['terminal_id']->$box->color = isset($numbers[$index]['color'])
                      ? $numbers[$index]['color'] : ''; // Added by PAG
                    $boxes->terminals->$number['terminal_id']->$box->user = (isset($boxes->show_names)
                      && $boxes->show_names)
                    && isset($numbers[$index]['name']) ? $numbers[$index]['name'] : ''; //Added by ARA
                }
            }
    ...
    public static function queueNumberActive($service_id, $priority_number, $next_number = null)
    {
        ...
        $count = PriorityNumber::where('priority_number.date', '=', $date)
          ...
          ->join('terminal_transaction', 'terminal_transaction.transaction_number', '=',
