    ...
    <div class="row">
      <div class="page-header clearfix">
        ...
        <div class="col-md-3 col-sm-4 ">
          ...
          <a id="view-broadcast" target="_blank" href="{{ url('/broadcast/business/' . $business_id) }}"><span
              class="glyphicon glyphicon-th-large"></span> View Broadcast Screen</a>
          <point-of-interest position="left" bottom="35" right="100" title="Broadcast Page"
                             description="Click on the <strong>View Broadcast Page</strong> link to view the numbers being called."></point-of-interest>
          <a ng-show="allowed_businesses.length > 0" id="forward-history" target="_blank"
             href="{{ url('/processqueue/forward-history/' . $service_id) }}"><span
              class="glyphicon glyphicon-share-alt"></span> View Forwarding History</a>
        </div>
      </div>
    </div>
    <div class="row">
              ...
              <form>
                <div class="col-md-8 col-sm-8 col-xs-9">
                  ...
                  <div class="dropdown-wrapper" ng-show="timebound_numbers.length != 0 || uncalled_numbers.length != 0">
                    ...
                    <ul class="dropdown-menu dd-select" id="uncalled-numbers">
                      ...
                      >
                        ...
                        <small class="font-normal" ng-if="number.queue_platform != 'web'">via <span
                            style="text-transform:capitalize;">@{{ number.queue_platform }}</span></small>
                                            <span
                                            ...
                                            >
                                                <a href="#">
                                                    ...
                                                    >@{{ number.name }}</span>
                                                    <span
                                                      ng-if="!number.checked_in && number.queue_platform =='remote'">
                                                        <small class="c-status pull-right mr5 font-normal">not checked in :</small>
                                                        <span
                                                          class="dpq-icons pull-right glyphicon glyphicon-remove"> </span>
                                                    </span>
                                                    <span ng-if="number.checked_in && number.queue_platform =='remote'">
                                                        ...
                                                        <span
                                                          class="dpq-icons pull-right checkedin glyphicon glyphicon-ok"> </span>
                                                    </span>
                                                </a>
                                            </span>
                      </li>
                  ...
                  <div ng-show="timebound_numbers.length == 0 && uncalled_numbers.length == 0">
                    <!-- process queue issue number textfield -->
                    <input ng-show="number_prefix == '' && number_suffix == '' " type="text"
                           class="form-control issue-call-number" min="1" max="@{{ number_limit }}"
                           ng-model="issue_call_number">
                    <div class="input-group mb10" ng-show="number_prefix != '' || number_suffix != '' ">
                      ...
                      <input type="text" class="form-control issue-call-number" min="1" max="@{{ number_limit }}"
                             ng-model="issue_call_number">
                ...
                <point-of-interest position="left" bottom="85" right="100" title="Issued Numbers"
                                   description="Look for the numbers you want to call in this drop-down list or type the number you want call when the list is empty."></point-of-interest>
                <div class="col-md-2 col-sm-2 col-xs-3">
                  <a ng-if="date == today" id="btn-pmore" class="btn btn-md btn-primary" data-toggle="modal"
                     data-target="#moreq" title="Issue a number.">+</a>
                  <a class="btn btn-md btn-primary" title="Call again." ng-click="sendWebsocket()">!</a>
                </div>
                <point-of-interest position="right" bottom="85" right="25" title="Issue Numbers"
                                   description="Click on the blue '+' (plus) button to issue more numbers."></point-of-interest>
                <div class="col-md-2 col-sm-2 col-xs-12 text-right">
                  ...
                  <button class="btn btn-orange btn-md" id="btn-call" ng-click="issueOrCall()" ng-disabled="isCalling"
                          ng-if="date == today">CALL NUMBER
                  </button>
                  <button class="btn btn-orange btn-md" id="btn-call" ng-click="moveToday()" ng-disabled="isCalling"
                          ng-if="date != today">MOVE TO CURRENT
                  </button>
                </div>
                <point-of-interest position="right" bottom="85" right="-1" title="Call Numbers"
                                   description="Click on the <strong>CALL NUMBER</strong> button to call the number selected on the drop-down list."></point-of-interest>
              </form>
            </div>
          </div>
          <div ng-show="called_numbers.length != 0">
            <point-of-interest position="left" bottom="68" right="96" title="Called Number"
                               description="Click on the number to view the information about the user assigned to this number."></point-of-interest>
            <point-of-interest position="left" bottom="68" right="16.5" title="Drop Number"
                               description="The <strong>Drop</strong> button (trashcan icon) indicates that the person assigned to the number did not show thus removes the number from the list."></point-of-interest>
            <point-of-interest position="left" bottom="68" right="1.5" title="Next Number"
                               description="The <strong>Next</strong> button indicates that the number has been served and calls the next number on the list."></point-of-interest>
          </div>
          <table class="table table-striped">
              ...
              <td>
                <div class="progress" style="padding-top: 0;" ng-show="isStopping">
                  <div class="progress-bar progress-bar-danger progress-bar-striped" role="progressbar"
                       aria-valuenow="@{{ progress_current }}" aria-valuemin="0" aria-valuemax="@{{ progress_max }}"
                       style="padding-top: 0; width: @{{ stop_progress + '%' }}">
                    <span> Stopping... </span>
                  </div>
                </div>
              </td>
              <td>
                <button ng-if="date == today" class="pull-right btn btn-sm btn-danger stopbutton"
                        ng-click="stopProcessQueue()">
            ...
            <tr ng-repeat="number in called_numbers" data-tnumber="@{{ number.transaction_number }}">
              <td scope="row" width="15%">
                <a href="#" class="priority-number" title="Number: @{{ number.priority_number }}"
                   data-transaction-number="@{{ number.transaction_number }}"
                   data-priority-number="@{{ number.priority_number }}" data-name="@{{ number.name }}"
                   data-phone="@{{ number.phone }}" data-email="@{{ number.email }}"
                   data-form_records="@{{ number.form_records }}"
                   data-confirmation-code="@{{ number.confirmation_code }}" data-toggle="modal"
                   data-target="#priority-number-modal">
                  @{{ number.priority_number }}
                </a>
              </td>
              <td width="15%" class="text-center">
                <span
                  class="@{{ number.terminal_id == terminal_id ? 'cyan' : 'red' }}">@{{ number.terminal_name }}</span>
              </td>
              <td class="links">
                <div class="pt0" ng-show="number.form_records.length">
                  <a href="#" class="btn btn-primary btn-xs btn-links" data-toggle="modal"
                     data-target="#modal-forms@{{ number.priority_number }}"><span
                      class="glyphicon glyphicon-list-alt"></span> View Form</a>
                  <div class="modal fade modal-forms" id="modal-forms@{{ number.priority_number }}" tabindex="-1"
                       role="dialog">
                        ...
                        <div class="modal-body pt0">
                          <div class="clearfix">
                            <ul id="tab-forms" class="clearfix nav nav-tabs pt0">
                              <li role="presentation" ng-repeat="form in number.form_records"
                            ...
                            <div id="tab-forms-table" class="pt0 tab-content">
                              <div role="tabpanel" ng-repeat="form in number.form_records"
                                   class="pt0 tab-pane fade @{{ $index == 0 ? 'active in':'' }}"
                                   id="service-@{{ $index }}">
                                <table class="table">
                                  <thead>
                                  <tr>
              ...
              <td width="20%">
                <form ng-if="date == today" class="star-rating-form" ng-show="called_numbers[$index].verified_email">
                                <span class="star-rating">
                                    <input type="radio" name="rating" ng-model="called_numbers_rating[$index]"
                                           value="1"><i></i>
                                    <input type="radio" name="rating" ng-model="called_numbers_rating[$index]"
                                           value="2"><i></i>
                                    <input type="radio" name="rating" ng-model="called_numbers_rating[$index]"
                ...
                <a ng-if="date == today" class="delete" ng-disabled="isProcessing" data-toggle="modal"
                   data-target="#drop-modal"><span class="glyphicon glyphicon-trash"></span></a>
                <a ng-if="date == today" class="btn btn-sm btn-default"
                   ng-click="serveAndCallNext(number.transaction_number)" ng-disabled="isProcessing">Next <span
                    ...
                    <div class="modal-content text-left">
                      ...
                      <div class="modal-footer">
                        <button type="button" data-dismiss="modal" class="btn btn-danger" id="delete"
                                ng-click="dropNumber(number.transaction_number)">Drop
                        </button>
