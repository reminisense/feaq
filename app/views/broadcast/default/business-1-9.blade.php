@include('broadcast.default.business-ads-master')

<div class="numbers-area {{ $num_class }} abs">
  <div class="boxed nine">
    <h3 class="abs">NOW SERVING</h3>
    <div class="row-5">
      <div class="col-2 t@{{ rank1 }}">
        <div class="numbers">
          <p class="terminal">@{{ service1 }}</p>
          <p class="terminal">@{{ name1 }}</p>
          <p class="callnum">@{{ box1 }}</p>
        </div>
      </div>
      <div class="col-2 t@{{ rank2 }}">
        <div class="numbers">
          <p class="terminal">@{{ service2 }}</p>
          <p class="terminal">@{{ name2 }}</p>
          <p class="callnum">@{{ box2 }}</p>
        </div>
      </div>
      <div class="col-2 t@{{ rank3 }}">
        <div class="numbers">
          <p class="terminal">@{{ service3 }}</p>
          <p class="terminal">@{{ name3 }}</p>
          <p class="callnum">@{{ box3 }}</p>
        </div>
      </div>
      <div class="col-2 t@{{ rank4 }}">
        <div class="numbers">
          <p class="terminal">@{{ service4 }}</p>
          <p class="terminal">@{{ name4 }}</p>
          <p class="callnum">@{{ box4 }}</p>
        </div>
      </div>
      <div class="col-2 t@{{ rank5 }}">
        <div class="numbers">
          <p class="terminal">@{{ service5 }}</p>
          <p class="terminal">@{{ name5 }}</p>
          <p class="callnum">@{{ box5 }}</p>
        </div>
      </div>
      <div class="col-2 t@{{ rank6 }}">
        <div class="numbers">
          <p class="terminal">@{{ service6 }}</p>
          <p class="terminal">@{{ name6 }}</p>
          <p class="callnum">@{{ box6 }}</p>
        </div>
      </div>
      <div class="col-2 t@{{ rank7 }}">
        <div class="numbers">
          <p class="terminal">@{{ service7 }}</p>
          <p class="terminal">@{{ name7 }}</p>
          <p class="callnum">@{{ box7 }}</p>
        </div>
      </div>
      <div class="col-2 t@{{ rank8 }}">
        <div class="numbers">
          <p class="terminal">@{{ service8 }}</p>
          <p class="terminal">@{{ name8 }}</p>
          <p class="callnum">@{{ box8 }}</p>
        </div>
      </div>
      <div class="col-1 t@{{ rank9 }}">
        <div class="numbers">
          <p class="terminal">@{{ service9 }}</p>
          <p class="terminal">@{{ name9 }}</p>
          <p class="callnum">@{{ box9 }}</p>
        </div>
      </div>
    </div>
  </div>
</div>